...
object SaveHbaseTraceUser2Kudu {
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder()
      .appName("SaveHbaseTraceUser2Kudu")
    ...
    //读取hbase数据
    val tableName = "db_user_tags_t_trace_user"
    ...
    val dataRDD = hbaseRDD
      ...
      .map { result =>
        ...
        (Bytes.toString(result.getRow),
          ...
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_amount_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_amount_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("yearly_income_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("yearly_income_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_experience_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_experience_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_time_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("investment_time_"))) })
      }
      .map(x => {
        RowFactory.create(
          x._1,
    ...
    structFields.add(DataTypes.createStructField("investment_amount_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("yearly_income_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("investment_experience_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("investment_time_", DataTypes.StringType, false))
    ...
    kuduContext.upsertRows(df.repartition(50), "db_user_tags.t_trace_user")
