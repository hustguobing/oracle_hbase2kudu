...
object SaveHbaseTgFollow2Kudu {
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder()
      .appName("SaveHbaseTgFollow2Kudu")
    ...
    //读取hbase数据
    val tableName = "db_user_tags_t_user_follow"
    ...
    val dataRDD = hbaseRDD
      ...
      .map { result =>
        ...
        (Bytes.toString(result.getRow),
          ...
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("tgName_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("tgName_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("follow_time_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("follow_time_"))) })
      }
      .map(x => {
        RowFactory.create(
          x._1,
    ...
    structFields.add(DataTypes.createStructField("tgName_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("follow_time_", DataTypes.LongType, false))
    ...
    kuduContext.upsertRows(df.repartition(50), "db_user_tags.t_user_follow")
