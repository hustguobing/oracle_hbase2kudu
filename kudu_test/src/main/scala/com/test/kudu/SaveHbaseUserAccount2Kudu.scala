...
object SaveHbaseUserAccount2Kudu {
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder()
      .appName("SaveHbaseUserAccount2Kudu")
    ...
    //读取hbase数据
    val tableName = "db_user_tags_t_user_account"
    ...
    val dataRDD = hbaseRDD
      ...
      .map { result =>
        ...
        (Bytes.toString(result.getRow),
          ...
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("trader_company_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("trader_company_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("open_success_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("open_success_"))) },
          if (result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("amount_type_")) == null) { "" } else { Bytes.toString(result.getValue(Bytes.toBytes("f1"), Bytes.toBytes("amount_type_"))) })
      }
      .map(x => {
        RowFactory.create(
          x._1,
    ...
    structFields.add(DataTypes.createStructField("trader_company_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("open_success_", DataTypes.StringType, false))
    structFields.add(DataTypes.createStructField("amount_type_", DataTypes.StringType, false))
    ...
    kuduContext.upsertRows(df.repartition(50), "db_user_tags.t_user_account")
